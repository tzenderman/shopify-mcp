...
// Input schema for creating a draft order
const CreateDraftOrderInputSchema = z.object({
  storeId: z.string().min(1).describe("The store ID to create draft order in"),
  lineItems: z.array(DraftOrderLineItemSchema).min(1).describe("Line items for the draft order"),
...
const createDraftOrder = {
  ...
  description: "Create a new draft order in a specific store",
  ...
  execute: async (input: CreateDraftOrderInput) => {
    try {
      const { storeId, ...draftOrderInput } = input;
      ...
      return { draftOrder: data.draftOrderCreate.draftOrder };
    } catch (error) {
